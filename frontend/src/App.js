...
const FLASK_PING_URL = (process.env.REACT_APP_API_URL || "http://localhost:5001").replace(/\/$/, "") + "/api/ping";
...
const WakeupScreen = ({ onReady }) => {
  ...
  useEffect(() => {
    ...
    const ping = async () => {
      try {
        const res = await fetch(FLASK_PING_URL, { method: "GET" });
        if (res.ok) {
          setStatus("✅ ML service is ready!");
          clearInterval(dotInterval);
          clearInterval(secInterval);
          setTimeout(onReady, 800); // small delay so user sees "ready"
          return;
        }
      } catch {
        setStatus("Waking up ML service" + dots);
      }
      setTimeout(ping, 3000); // retry every 3 seconds
    };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {isReady ? "✅ ML SERVICE IS READY!" : `WAKING UP ML SERVICE${dots}`}
      </div>
      ...
      {!isReady && (
        <div style={{
          ...
          <div style={{
            ...
            This app runs on <span style={{ color: "#666" }}>Render's free tier</span>.
            The ML service sleeps after inactivity and takes{" "}
            <span style={{ color: "#fbbf24" }}>15–30 seconds</span> to wake up.
            <br /><br />
